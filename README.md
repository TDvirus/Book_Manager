**本项目基于牛客网项目制作，主要用来对所学知识的检验和巩固作用，原地址在此[BookManager](https://git.nowcoder.com/11000160/BookManager)**



*本项目在原项目的基础上进行了如下改动：*

*1.添加了lombok自动生成getter/setter/toString方法,移除了joda-time包改用了JDK1.8的LocalDateTime类进行日期的处理；*

*2.改写pojo字段和数据库表的字段适应驼峰规则；*

*3.改进了原项目中注册第二个账户时发生数据库主键冲突报错的问题（采用的方法是捕获异常但不处理）。*

**还有模仿的过程中踩了个坑：mybatis代理的interface接口不允许有重载方法，否则会报错**
